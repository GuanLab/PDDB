...
def network(input_var, label_var, shape):
    ...
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=32, filter_size=5)) # 992
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 496
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=32, filter_size=5)) # 492
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 246
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=64, filter_size=5)) # 242
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 121
