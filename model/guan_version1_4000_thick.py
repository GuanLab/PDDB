...
def network(input_var, label_var, shape):
    ...
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=32, filter_size=5))  # 3996
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 1998
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=32, filter_size=5)) # 1994
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 997
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=64, filter_size=4)) # 994
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 497
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=64, filter_size=4)) # 494
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 247
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=128, filter_size=4)) # 244
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 122
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=128, filter_size=4)) # 118
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 59
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=256, filter_size=4)) # 56
    layer = nn.layers.MaxPool1DLayer(layer, pool_size=2)                    # 28
    layer = bn(nn.layers.Conv1DLayer(layer, num_filters=256, filter_size=5)) # 24
